(function(){
  ...
  const pageLine=()=>{
    ...
    const table=[
      ...
      ['minglan-network-civility','旧论坛镜像保留年份、原作者和原题目；正文没有另加专题标签。'],
      ...
      ['press-tree-care','YL-E-017在公开资料里登记为普通垂柳；2014底片索引另记位置、时间和画面人数。'],
      ...
      ['hq-migrate','2012—2016旧库正文未建全文索引，event_date 与 area 字段按迁移时原值保留。'],
      ...
      ['oa-index','2017-01迁移批次未重建正文索引，公开查询只保留 event_date 与 area 两项字段。'],
      ['oa-incidents','REC_QUERY读取 event_date / area；零点后抓取的论坛缓存可能保留空日期字段。'],
      ...
      ['oa-server-path','2017-01-09静态盘点：业务目录之外另保留1个owner为空的HTML。']
    ];
