(function(){
 ...
 if(form){const date=document.getElementById('qDate'),area=document.getElementById('qArea'),box=document.getElementById('queryResults'),msg=document.getElementById('queryMessage'),count=document.getElementById('queryCount'),body=document.getElementById('queryBody'),echo=document.getElementById('queryEcho');const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  form.addEventListener('submit',e=>{e.preventDefault();const d=(date.value||'').trim(),a=area.value||'';if(!d&&!a){box.hidden=true;msg.textContent='error: at least one field required.';return;}if(d&&!/^\d{4}-\d{2}-\d{2}$/.test(d)){box.hidden=true;msg.textContent='error: invalid date format. use YYYY-MM-DD.';return;}const rows=data.filter(x=>(!d||x.date===d)&&(!a||x.area===a));body.innerHTML=rows.map(x=>`<tr><td>${x.href?`<a href="${esc(x.href)}">${esc(x.id)}</a>`:esc(x.id)}</td><td>${esc(x.time)}</td><td>${esc(labels[x.area]||x.area)}</td><td>${esc(x.source)}</td><td>${esc(x.summary)}</td></tr>`).join('');box.hidden=false;count.textContent=String(rows.length);echo.textContent='> '+(d||'*')+' / '+(labels[a]||'*');msg.textContent=rows.length?'query complete.':'0 records.';});
  document.getElementById('queryReset')?.addEventListener('click',()=>{date.value='';area.value='';box.hidden=true;msg.textContent='ready.';});
 }
 const tabs=[...document.querySelectorAll('[data-oa-tab]')];if(tabs.length){const sources=[...document.querySelectorAll('[data-oa-source]')],selected=document.getElementById('selectedRecord');function open(key){const btn=tabs.find(b=>b.dataset.oaTab===key)||tabs[0];tabs.forEach(b=>b.classList.toggle('active',b===btn));sources.forEach(s=>s.classList.toggle('active',s.dataset.oaSource===btn.dataset.oaTab));if(selected)selected.textContent=btn.dataset.recordId||'';}tabs.forEach(btn=>btn.addEventListener('click',()=>open(btn.dataset.oaTab)));const hash=location.hash.replace('#','');open(hash&&tabs.some(b=>b.dataset.oaTab===hash)?hash:tabs[0].dataset.oaTab);}
})();
