(function(){
 ...
 function apply(){const q=(search?.value||'').trim().toLowerCase();let n=0;items.forEach(x=>{const okYear=q?true:(year==='all'||x.dataset.year===year);const author=(x.lastElementChild?.textContent||'').trim().toLowerCase();const okText=!q||author.includes(q);const ok=okYear&&okText;x.style.display=ok?'grid':'none';if(ok)n++;});if(stat)stat.textContent=(q?'用户名搜索':(year==='all'?'全部年份':year))+' · '+n+' 条';}
